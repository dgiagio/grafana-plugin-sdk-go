...
// ChunkedDataHandler defines the interface for handling chunked data requests.
type ChunkedDataHandler interface {
	...
	QueryChunkedData(ctx context.Context, req *ChunkedDataRequest, w ChunkedDataWriter) error
}
...
type ChunkedDataWriter interface {
	...
	WriteError(refID string, err error) error
...
// ChunkedDataHandlerFunc is an adapter to allow the use of
// ordinary functions as [ChunkedDataHandler]. If f is a function
// with the appropriate signature, ChunkedDataHandlerFunc(f) is a
// [ChunkedDataHandler] that calls f.
type ChunkedDataHandlerFunc func(ctx context.Context, req *ChunkedDataRequest, w ChunkedDataWriter) error

// QueryChunkedData calls fn(ctx, req).
func (fn ChunkedDataHandlerFunc) QueryChunkedData(ctx context.Context, req *ChunkedDataRequest, w ChunkedDataWriter) error {
	return fn(ctx, req, w)
}

// ChunkedDataRequest contains all information needed for a chunked data request.
...
// associated with key. The key is case-insensitive; it is
// canonicalized by textproto.CanonicalMIMEHeaderKey.
func (req *ChunkedDataRequest) SetHTTPHeader(key, value string) {
	if req.Headers == nil {
		req.Headers = map[string]string{}
	}
...
// The key is case-insensitive; it is canonicalized by
// CanonicalHeaderKey.
func (req *ChunkedDataRequest) DeleteHTTPHeader(key string) {
	deleteHTTPHeaderInStringMap(req.Headers, key)
}
...
// used to canonicalize the provided key. Get assumes that all
// keys are stored in canonical form.
func (req *ChunkedDataRequest) GetHTTPHeader(key string) string {
	return req.GetHTTPHeaders().Get(key)
}

// GetHTTPHeaders returns HTTP headers.
func (req *ChunkedDataRequest) GetHTTPHeaders() http.Header {
	return getHTTPHeadersFromStringMap(req.Headers)
}
...
var _ ForwardHTTPHeaders = (*ChunkedDataRequest)(nil)
