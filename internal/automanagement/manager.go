...
func (m *Manager) QueryChunkedData(ctx context.Context, req *backend.ChunkedDataRequest, w backend.ChunkedDataWriter) error {
	h, err := m.Get(ctx, req.PluginContext)
	if err != nil {
		return err
	}
	if ds, ok := h.(backend.ChunkedDataHandler); ok {
