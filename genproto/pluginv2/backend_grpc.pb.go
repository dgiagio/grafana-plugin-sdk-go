...
type DataClient interface {
	...
	QueryChunkedData(ctx context.Context, in *ChunkedDataRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ChunkedDataResponse], error)
}
...
func (c *dataClient) QueryChunkedData(ctx context.Context, in *ChunkedDataRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ChunkedDataResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Data_ServiceDesc.Streams[0], Data_QueryChunkedData_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ChunkedDataRequest, ChunkedDataResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Data_QueryChunkedDataClient = grpc.ServerStreamingClient[ChunkedDataResponse]
...
type DataServer interface {
	...
	QueryChunkedData(*ChunkedDataRequest, grpc.ServerStreamingServer[ChunkedDataResponse]) error
}
...
func (UnimplementedDataServer) QueryChunkedData(*ChunkedDataRequest, grpc.ServerStreamingServer[ChunkedDataResponse]) error {
...
func _Data_QueryChunkedData_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ChunkedDataRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(DataServer).QueryChunkedData(m, &grpc.GenericServerStream[ChunkedDataRequest, ChunkedDataResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Data_QueryChunkedDataServer = grpc.ServerStreamingServer[ChunkedDataResponse]
